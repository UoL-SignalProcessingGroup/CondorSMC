data {
  ...
  array[Nobs] int<lower=1, upper=N> obsInd;
  array[Nmiss] int<lower=1, upper=N> missInd;
  ...
  array[N] int<lower=1,upper=nNormalisationLevels> normalisationMap;
  ...
  array[nPeptideRuns+1] int<lower=0,upper=nFeatures+nPeptideRuns> csAllFeatures;
  array[nPeptideRuns+1] int<lower=0,upper=nFeatures> csObsFeatures;
  array[nPeptideRuns] int<lower=0,upper=nFeatures> nFeaturesInPeptide;

  array[nProteins*nSamples] int<lower=1,upper=nProteins> proteinMap;

  //Selection of effect levels for each data point
  array[N] int<lower=1,upper=nFeatures+nPeptideRuns> featureMap;
  array[N] int<lower=1,upper=nDigestEffects> peptideDigestMap;

  int<lower=1,upper=nPeptides*nSamples*nProteins*nSamples> nNonZeroElts;
  array[nNonZeroElts] int<lower=1,upper = nPeptides*nSamples> lhs;
  array[nNonZeroElts] int<lower=1,upper = nProteins*nSamples> rhs;
  ...
  array[N] int<lower=1,upper=nPeptides*nSamples> peptideSampleMap;

  array[N] int<lower=1,upper=nResidualVarianceLevels> residualVarianceMap; //residual variance selection

  array[nDigestEffects] int<lower=1,upper=nPeptideVarianceLevels> digestEffectMap; //used only for generated quantities
