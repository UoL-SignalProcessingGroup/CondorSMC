...
class RWSMCSampler(SMCBase):
    """Random Walk SMC Sampler

    An SMC sampler that uses a Gaussian Random Walk to sample
    from the target distribution of interest.

    Attributes:
        K: Number of iterations.
        N: Number of particles.
        dim: Dimensionality of the target distribution.
    ...
    def sample(self, save_samples=False):
        """
        ...
        for k in tqdm(range(self.K)):
            ...
            x_new = self.forward_kernel.rvs(x, self.N)

            # Evaluate the target distribution, l kernel and forward kernel
            p_logpdf_x = self.target.logpdf(x)
            p_logpdf_xnew = self.target.logpdf(x_new)
            if self.lkernel:
                lkernel_logpdf = self.lkernel.calculate_rw(x, x_new)
                q_logpdf = self.forward_kernel.logpdf(x, x_new)
